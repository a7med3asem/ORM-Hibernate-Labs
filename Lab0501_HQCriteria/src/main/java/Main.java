...
//        }
        ...
        Criterion nameCriterion = Restrictions.eq("name", "BMW");
        Criteria criteria1 = session.createCriteria(Product.class);
        criteria1 = criteria1.add(nameCriterion);
//        Product product = (Product) criteria1.uniqueResult();
        printResult(criteria1.list());


        // question
        System.out.println("\n\n\n\n Q3 \n\n\n");
        Criteria criteria2 = session.createCriteria(User.class)
                .add(Restrictions.eq("userName", "Asim"));
        printResult(criteria2.list());


        System.out.println("\n\n\n\n Q4 \n\n\n");
        printResult(session.createCriteria(Product.class)
                .add(Restrictions.between("quantity", 100, 300))
                .list());

        System.out.println("\n\n\n\n Q5 \n\n\n");
        printResult(session.createCriteria(Product.class)
                .add(Restrictions.gt("quantity", 100))
                .list());

        System.out.println("\n\n\n\n Q6 \n\n\n");
        String[] emails = {"ahmedasem@gmail.com", "ahmedasem@gmail.com"};
        printResult(session.createCriteria(User.class)
        ...
        System.out.println("\n\n\n\n Q8 \n\n\n");
        printResult(session.createCriteria(Seller.class)
        ...
        System.out.println("\n\n\n\n Q10 \n\n\n");
        printResult(session.createCriteria(User.class)
                .add(Restrictions.like("userName", "ah%").ignoreCase())
                .list());

        System.out.println("\n\n\n\n Q11 \n\n\n");
        printResult(session.createCriteria(User.class)
                .add(Restrictions.like("userName", "ah", MatchMode.START))
                .list());
        ...
        printResult(session.createCriteria(User.class)
                .add(Restrictions.or(
                        Restrictions.and(
                                Restrictions.like("userName", "Mo", MatchMode.START),
                                Restrictions.like("userName", "Al", MatchMode.START)
                        ),
        ...
        Criteria criteria4 = session.createCriteria(Seller.class, "s")
                .add(Subqueries.gt(5L, subQuery));
        printResult(criteria4.list());
...
//                Restrictions.like("user.userName", "Ali", MatchMode.ANYWHERE));
//
//        Criteria productCriteria = sellerCriteria.createCriteria("products");
//
//        productCriteria = productCriteria.add(
//                Restrictions.like("name", "BMW", MatchMode.ANYWHERE));
//        printResult(productCriteria.list());
        ...
        printResult(session.createCriteria(Seller.class)
                ...
                .setProjection(
                        Projections.projectionList()
                                ...
                                .add(Property.forName("p.buyerBidProducts").count())).list());
    ...
    private static void printResult(List list) {
        ...
        iterator.forEachRemaining(o -> {
            if (o instanceof Product) {
                ...
                System.out.println(seller.getId() + "  " + seller.getUser().getUserName());
            }
        });
    }
}
